...
import { AGENTS_PATH, AgentVaultStorage } from './AgentVaultStorage.js';
import { CC_SETTINGS_PATH, CCSettingsStorage, isLegacyPermissionsFormat } from './CCSettingsStorage.js';
import {
...
} from './migrationConstants.js';
import { SESSIONS_PATH, SessionStorage } from './SessionStorage.js';
import { SKILLS_PATH, SkillStorage } from './SkillStorage.js';
import { COMMANDS_PATH, SlashCommandStorage } from './SlashCommandStorage.js';
import { VaultFileAdapter } from './VaultFileAdapter.js';

/** Base path for all Claudian storage. */
export const CLAUDE_PATH = '.archivist';

/** Path for plugin-data.json (replaces Obsidian's data.json). */
const PLUGIN_DATA_PATH = `${CLAUDE_PATH}/plugin-data.json`;
...
export class StorageService {
  ...
  private notifications: NotificationEmitter;

  constructor(graphRoot: string, notifications: NotificationEmitter) {
    this.graphRoot = graphRoot;
    this.notifications = notifications;
    this.adapter = new VaultFileAdapter(graphRoot);
  ...
  private async clearLegacyDataJson(): Promise<void> {
    ...
    await this.saveDataJson(cleaned);
  }

  private async loadDataJson(): Promise<LegacyDataJson | null> {
    try {
      if (!(await this.adapter.exists(PLUGIN_DATA_PATH))) {
        return null;
      }
      const content = await this.adapter.read(PLUGIN_DATA_PATH);
      const data = JSON.parse(content);
      return data || null;
    } catch {
      // plugin-data.json may not exist on fresh installs
      return null;
    }
  }

  private async saveDataJson(data: unknown): Promise<void> {
    await this.adapter.write(PLUGIN_DATA_PATH, JSON.stringify(data, null, 2));
  }
  ...
  async getTabManagerState(): Promise<TabManagerPersistedState | null> {
    try {
      ...
      if (data && (data as Record<string, unknown>).tabManagerState) {
        return this.validateTabManagerState((data as Record<string, unknown>).tabManagerState);
      }
      return null;
    } catch {
      return null;
  ...
  async setTabManagerState(state: TabManagerPersistedState): Promise<void> {
    try {
      const data = ((await this.loadDataJson()) as Record<string, unknown>) || {};
      data.tabManagerState = state;
      await this.saveDataJson(data);
    } catch {
      this.notifications.warn('Failed to save tab layout');
    }
  }
}
