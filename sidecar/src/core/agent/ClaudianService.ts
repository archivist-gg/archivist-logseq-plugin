...
import type { NotificationEmitter } from '../../adapter/index.js';
import { stripCurrentNoteContext } from '../utils/context.js';
import { getEnhancedPath, getMissingNodeError, parseEnvironmentVariables } from '../utils/env.js';
import { getPathAccessType } from '../utils/path.js';
import {
...
import type { ClaudianSettings } from '../types/settings.js';
import type { StorageService } from '../storage/StorageService.js';
import type { PluginManager } from '../plugins/PluginManager.js';
import type { AgentManager } from '../agents/AgentManager.js';

/**
 * Abstraction over the Obsidian InquiryModule plugin, allowing the sidecar
 * to provide equivalent functionality without Obsidian dependencies.
 */
export interface SidecarContext {
  vaultPath: string;
  notifications: NotificationEmitter;
  getSettings(): ClaudianSettings;
  getResolvedClaudeCliPath(): string;
  ...
  /** Get Archivist host settings (for D&D context). */
  getArchivistSettings(): { ttrpgRootDir?: string };
}
import {
  createBlocklistHook,
...
import { isSessionInitEvent, isStreamChunk, transformSDKMessage } from '../sdk/index.js';
import {
  buildPermissionUpdates,
  getActionDescription,
} from '../security/index.js';
import { TOOL_ASK_USER_QUESTION, TOOL_ENTER_PLAN_MODE, TOOL_EXIT_PLAN_MODE, TOOL_SKILL } from '../tools/toolNames.js';
import type {
  ApprovalDecision,
  ChatMessage,
  Conversation,
...
import { isAdaptiveThinkingModel } from '../types/index.js';
...
export class ClaudianService {
  ...
  async ensureReady(options?: EnsureReadyOptions): Promise<boolean> {
    const vaultPath = this.context.vaultPath;
    ...
    if (!this.persistentQuery) {
      ...
      const cliPath = this.context.getResolvedClaudeCliPath();
    ...
    if (this.needsRestart(newConfig)) {
      ...
      const cliPathAfterClose = this.context.getResolvedClaudeCliPath();
  ...
  private async startPersistentQuery(
    ...
    if (resumeSessionId) {
      ...
      this.sessionManager.setSessionId(resumeSessionId, this.context.getSettings().model);
    }
  ...
  private buildQueryOptionsContext(vaultPath: string, cliPath: string): QueryOptionsContext {
    const customEnv = parseEnvironmentVariables(this.context.getActiveEnvironmentVariables());
    const enhancedPath = getEnhancedPath(customEnv.PATH, cliPath);

    // Build D&D context from Archivist host settings
    const archivistSettings = this.context.getArchivistSettings();
    ...
    return {
      ...
      archivistMcpServer: this.context.archivistMcpServer,
      dndContext,
    };
  }
  ...
  private buildHooks(externalContextPaths?: string[]) {
    const blocklistHook = createBlocklistHook(() => ({
      blockedCommands: this.context.getSettings().blockedCommands,
      enableBlocklist: this.context.getSettings().enableBlocklist,
    }));

    const hooks: Options['hooks'] = {};

    if (this.context.getSettings().allowExternalAccess) {
      ...
      const vaultRestrictionHook = createVaultRestrictionHook({
        getPathAccessType: (p) => {
          ...
          return getPathAccessType(
            p,
            paths,
            this.context.getSettings().allowedExportPaths,
            this.vaultPath
          );
        },
      });
  ...
  private getTransformOptions(modelOverride?: string) {
    return {
      intendedModel: modelOverride ?? this.context.getSettings().model,
      customContextLimits: this.context.getSettings().customContextLimits,
    };
  }
  ...
  private async routeMessage(message: SDKMessage): Promise<void> {
    ...
    for (const event of transformSDKMessage(message, this.getTransformOptions())) {
      if (isSessionInitEvent(event)) {
        ...
        if (event.agents) {
          try { this.context.agentManager.setBuiltinAgentNames(event.agents); } catch { /* non-critical */ }
        }
    ...
    if (isTurnCompleteMessage(message)) {
      ...
      if (handler) {
        ...
        try {
          ...
          this.context.notifications.warn('Background task completed, but the result could not be rendered.');
        }
      }
    }
  }
  ...
  async *query(
    ...
    const resolvedClaudePath = this.context.getResolvedClaudeCliPath();
  ...
  private async applyDynamicUpdates(
    ...
    const selectedModel = queryOptions?.model || this.context.getSettings().model;
    const permissionMode = this.context.getSettings().permissionMode;
    ...
    if (this.currentConfig && selectedModel !== this.currentConfig.model) {
      try {
        ...
        this.context.notifications.warn('Failed to update model');
      }
    }
    ...
    if (this.currentConfig && permissionMode !== this.currentConfig.permissionMode) {
      ...
      try {
        ...
        this.context.notifications.warn('Failed to update permission mode');
      }
    }
    ...
    if (this.currentConfig && mcpServersKey !== this.currentConfig.mcpServersKey) {
      ...
      if (hasUserServers || hadUserServers) {
        try {
          ...
          this.context.notifications.warn('Failed to update MCP servers');
        }
      }
  ...
  setSessionId(id: string | null, externalContextPaths?: string[]): void {
    ...
    this.sessionManager.setSessionId(id, this.context.getSettings().model);
    ...
    return async (toolName, input, options): Promise<PermissionResult> => {
      ...
      if (toolName === TOOL_EXIT_PLAN_MODE && this.exitPlanModeCallback) {
        try {
          ...
          const sdkMode = this.mapToSDKPermissionMode(this.context.getSettings().permissionMode);
          // Sync config so applyDynamicUpdates doesn't re-send
          if (this.currentConfig) {
            this.currentConfig.permissionMode = this.context.getSettings().permissionMode;
          }
          return {
            behavior: 'allow',
            updatedInput: input,
