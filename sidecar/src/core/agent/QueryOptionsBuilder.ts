...
import { buildSystemPrompt, type SystemPromptSettings } from '../prompts/mainAgent.js';
import type { DndPromptContext } from '../prompts/dndContext.js';
import type { ClaudianSettings } from '../types/index.js';
import { isAdaptiveThinkingModel } from '../types/index.js';
import { createCustomSpawnFunction } from './customSpawn.js';
import {
...
export class QueryOptionsBuilder {
  ...
  private static applyThinking(
    ...
    if (isAdaptiveThinkingModel(model)) {
      (options as Record<string, unknown>).thinking = { type: 'adaptive' };
      (options as Record<string, unknown>).effort = settings.effortLevel;
    }
  }
