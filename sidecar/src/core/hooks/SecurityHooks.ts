...
import type { PathAccessType } from '../utils/path.js';
import type { PathCheckContext } from '../security/BashPathValidator.js';
import { findBashCommandPathViolation } from '../security/BashPathValidator.js';
import { isCommandBlocked } from '../security/BlocklistChecker.js';
import { getPathFromToolInput } from '../tools/toolInput.js';
import { isEditTool, isFileTool, TOOL_BASH } from '../tools/toolNames.js';
import { getBashToolBlockedCommands, type PlatformBlockedCommands } from '../types/index.js';
    ...
    hooks: [
      async (hookInput) => {
        ...
        if (isCommandBlocked(command, bashToolCommands, context.enableBlocklist)) {
          context.onBlocked?.('Command blocked by security policy');
          return {
            continue: false,
            hookSpecificOutput: {
