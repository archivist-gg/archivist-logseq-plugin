...
import type { CCSettingsStorage } from '../storage/CCSettingsStorage.js';
import type { ClaudianPlugin, InstalledPluginEntry, InstalledPluginsFile, PluginScope } from '../types/index.js';
    ...
    if (installedPlugins?.plugins) {
      for (const [pluginId, entries] of Object.entries(installedPlugins.plugins)) {
        ...
        if (!Array.isArray(entries)) {
          console.warn(`Claudian: plugin "${pluginId}" has malformed entry in installed_plugins.json (expected array, got ${typeof entries})`);
        }
