/**
 * BangBashModeManager -- ! mode UI for direct bash execution.
 *
 * Ported from Obsidian. Changes:
 * - `Notice` from obsidian -> console.error (no toast in Logseq iframe)
 * - `t()` i18n import from local i18n module
 * - `addClass`/`removeClass` -> `classList.add`/`classList.remove`
 * - Bash execution goes through sidecar, not locally
 */

import { t } from '../i18n';
import type { InstructionInputLike } from './InstructionMode';

export interface BangBashModeCallbacks {
  onSubmit: (command: string) => Promise<void>;
  getInputWrapper: () => HTMLElement | null;
  resetInputHeight?: () => void;
}

export interface BangBashModeState {
  active: boolean;
  rawCommand: string;
}

export class BangBashModeManager {
  private input: InstructionInputLike;
  private callbacks: BangBashModeCallbacks;
  private state: BangBashModeState = { active: false, rawCommand: '' };
  private isSubmitting = false;
  private originalPlaceholder: string = '';

  constructor(
    input: InstructionInputLike,
    callbacks: BangBashModeCallbacks
  ) {
    this.input = input;
    this.callbacks = callbacks;
    this.originalPlaceholder = input.getPlaceholder();
  }

  handleTriggerKey(e: KeyboardEvent): boolean {
    if (!this.state.active && this.input.getValue() === '' && e.key === '!') {
      if (this.enterMode()) {
        e.preventDefault();
        return true;
      }
    }
    return false;
  }

  handleInputChange(): void {
    if (!this.state.active) return;
    this.state.rawCommand = this.input.getValue();
  }

  private enterMode(): boolean {
    const wrapper = this.callbacks.getInputWrapper();
    if (!wrapper) return false;

    wrapper.classList.add('claudian-input-bang-bash-mode');
    this.state = { active: true, rawCommand: '' };
    this.input.setPlaceholder(t('chat.bangBash.placeholder'));
    return true;
  }

  private exitMode(): void {
    const wrapper = this.callbacks.getInputWrapper();
    if (wrapper) {
      wrapper.classList.remove('claudian-input-bang-bash-mode');
    }
    this.state = { active: false, rawCommand: '' };
    this.input.setPlaceholder(this.originalPlaceholder);
  }

  handleKeydown(e: KeyboardEvent): boolean {
    if (!this.state.active) return false;

    if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
      e.preventDefault();
      if (this.state.rawCommand.trim()) {
        this.submit();
      }
      return true;
    }

    if (e.key === 'Escape' && !e.isComposing) {
      e.preventDefault();
      this.clear();
      return true;
    }

    return false;
  }

  isActive(): boolean {
    return this.state.active;
  }

  getRawCommand(): string {
    return this.state.rawCommand;
  }

  private async submit(): Promise<void> {
    if (this.isSubmitting) return;

    const rawCommand = this.state.rawCommand.trim();
    if (!rawCommand) return;

    this.isSubmitting = true;

    try {
      this.clear();
      await this.callbacks.onSubmit(rawCommand);
    } catch (e) {
      console.error(`[archivist] Command failed: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      this.isSubmitting = false;
    }
  }

  clear(): void {
    this.input.setValue('');
    this.exitMode();
    this.callbacks.resetInputHeight?.();
  }

  destroy(): void {
    this.exitMode();
  }
}
