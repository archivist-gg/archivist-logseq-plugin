 ...
 * - `container.createDiv()` -> `doc.createElement('div')` + `appendChild()`
 * - Constructor takes `doc: Document` + `container: HTMLElement`
 */
...
export class RichInput {
  ...
  constructor(doc: Document, container: HTMLElement, options: RichInputOptions = {}) {
    this.doc = doc;
    this.el = doc.createElement('div') as HTMLDivElement;
    this.el.className = 'claudian-rich-input';
    this.el.setAttribute('contenteditable', 'true');
    this.el.setAttribute('role', 'textbox');
    this.el.setAttribute('aria-multiline', 'true');
    this.el.setAttribute('dir', 'auto');
    this.el.dataset.placeholder = options.placeholder ?? '';
  ...
  removeTextBeforeCursor(count: number): void {
    ...
    this.doc.execCommand('delete', false);
  }
  ...
  insertEntityChip(entityType: string, name: string): void {
    ...
    const typeSpan = this.doc.createElement('span');
    typeSpan.className = 'archivist-inline-chip-type';
    typeSpan.textContent = entityType;
    chip.appendChild(typeSpan);

    const nameSpan = this.doc.createElement('span');
    nameSpan.className = 'archivist-inline-chip-name';
  ...
  insertFileChip(path: string, displayName: string): void {
    ...
    iconSpan.className = 'archivist-inline-chip-icon';
  ...
  private appendRemoveButton(chip: HTMLSpanElement): void {
    ...
    removeBtn.className = 'archivist-inline-chip-remove';
  ...
  private insertZeroWidthSpace(): void {
    const textNode = this.doc.createTextNode(ZERO_WIDTH_SPACE);
