/**
 * InstructionModeManager -- # mode UI for saving custom system prompts.
 *
 * Ported from Obsidian. Changes:
 * - Removed `obsidian` imports
 * - `addClass`/`removeClass` -> `classList.add`/`classList.remove`
 * - Otherwise nearly verbatim (pure DOM logic)
 */
...
export class InstructionModeManager {
  ...
  private enterMode(): boolean {
    ...
    wrapper.classList.add('claudian-input-instruction-mode');
  ...
  private exitMode(): void {
    const wrapper = this.callbacks.getInputWrapper();
    if (wrapper) {
      wrapper.classList.remove('claudian-input-instruction-mode');
    }
