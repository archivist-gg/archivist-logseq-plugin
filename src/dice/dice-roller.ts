...
export class DiceRoller {
  ...
  getShapes(index?: number): DiceShape[] {
  ...
  keepLow(drop: number = 1) {
    if (!this.modifiersAllowed) {
      console.warn("[archivist dice]", "Modifiers are only allowed on dice rolls.");
      return;
    }
  ...
  async reroll(times: number, conditionals: Conditional[]) {
    ...
    while (i < times && toReroll.length > 0) {
      const promises = [];
      for (const [_i, roll] of toReroll) {
        promises.push(
          new Promise<void>(async (resolve) => {
            roll.modifiers!.add("r");
            const newValue = this.getValueSync();
            roll.value = newValue;
  ...
  private resolveShapeValue(shapes: DiceShape[]): number {
  ...
  async roll(): Promise<void> {
    ...
    if (this.static) {
      ...
      // Launch all dice simultaneously (like the original's Promise.all pattern)
      const promises = [];
      for (let index = 0; index < this.rolls; index++) {
        promises.push(
          this.getValue(this.getShapes(index)).then((value) => {
  ...
  async makeUnique() {
    ...
    while (
      ...
      for (const [_index, result] of this.results) {
        promises.push(
          new Promise<void>(async (resolve) => {
  ...
  checkCondition(value: number, conditions: Conditional[]): boolean | number {
    ...
    for (const condition of conditions) {
      ...
      if (BasicStackRollerClass) {
        const roller = new BasicStackRollerClass(comparer, lexemes);
        roller.rollSync();
