...
class LexerClass {
    ...
    lexer = moo.compile({
        ...
        "!!": { match: /!!(?:i|\d+)?/u, value: this.clampInfinite },
        "!": { match: /!(?:i|\d+)?/u, value: this.clampInfinite },
        r: { match: /r(?:i|\d+)?/u, value: this.clampInfinite },
        u: /u/u,
        stunt: /1[Dd]S/u,
        "%": /\d+[Dd]\d+%/u,
        fudge: {
    ...
    parse(input: string): ParseResult<LexicalToken[]> {
        if (!input || !input.trim()) {
        ...
        try {
            ...
            this.lexer.reset();
            return { success: true, data: this.parser.parse(this.transform(tokens)) };
        } catch (e: any) {
            console.error(e);
